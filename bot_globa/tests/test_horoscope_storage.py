...
def test_horoscope_result_and_facts_round_trip_through_json_storage() -> None:
    ...
    result = AstrologyReadingResult.model_validate_json(json.dumps(valid_horoscope_payload(facts)))

    restored_result, restored_facts = deserialize_horoscope(serialize_horoscope(result, facts))
...
def test_memory_source_contains_narrative_but_no_calculated_fact_metadata() -> None:
    ...
    assert memory_source["interpretations"] == [item.text for item in result.interpretations]
