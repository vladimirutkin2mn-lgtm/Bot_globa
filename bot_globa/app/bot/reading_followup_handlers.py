...
PROMPT = (
    "Задайте уточняющий вопрос по этому разбору. Сеанс действует 24 часа после полного "
    "разбора, дополнительные списания не нужны.\n\nОсталось вопросов в сеансе: {remaining}."
)
NOT_ELIGIBLE = "Сеанс с уточнениями доступен только для оплаченного полного разбора."
SESSION_EXHAUSTED = "Сеанс завершён: 3 уточняющих вопроса уже использованы."
SESSION_EXPIRED = (
    "24 часа для уточняющих вопросов уже прошли. Сам полный разбор остаётся доступен "
    "в «Моих историях». Для новой ситуации можно начать новый разбор."
)
    ...
    async def cancel(self, callback: CallbackQuery, state: FSMContext) -> None:
        ...
        if isinstance(callback.message, Message):
            await show_screen(
                ...
                Scene.MAIN_MENU,
                (
                    "Сеанс приостановлен. Вернуться к нему можно из полного разбора "
                    "в течение 24 часов."
                ),
                reply_markup=main_menu_keyboard(),
                state=state,
