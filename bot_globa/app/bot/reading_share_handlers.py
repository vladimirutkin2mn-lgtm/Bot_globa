...
Only the dedicated anonymous `share_card` from a validated reading can leave the private
reading flow. Public URLs contain an aggregate acquisition marker only; reading ids stay
inside owner-gated Telegram callbacks and never enter product analytics metadata.
"""
...
SHARE_RENDERER_VERSION = "personal_share_v2"
SHARE_ENTRY_PAYLOAD = "share"
SHARE_PREVIEW_PREFIX = f"{SHARE_NAMESPACE}:preview:"
SHARE_CONFIRM_PREFIX = f"{SHARE_NAMESPACE}:confirm:"
MAX_PUBLIC_SHARE_LENGTH = 470

SHARE_PROMPT = (
    "✨ Похоже, этот разбор попал в точку. Можно отправить другу один анонимный инсайт — "
    "без вашего вопроса и полного разбора."
)
SHARE_LANDING = (
...
SHARE_PREVIEW_INTRO = (
    ...
    "Ниже — именно тот текст, который можно отправить. Ваш вопрос и полный разбор туда "
    "не попадут.\n\n"
)
SHARE_READY = "Карточка готова. Нажмите «Выбрать чат» — Telegram откроет меню отправки."
SHARE_UNAVAILABLE = "Эта карточка сейчас недоступна. Откройте полный разбор из «Моих историй»."


def share_offer_keyboard(reading_id: UUID) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="✨ Поделиться инсайтом",
                    callback_data=f"{SHARE_PREVIEW_PREFIX}{reading_id}",
                )
            ],
            [InlineKeyboardButton(text="Оставить себе", callback_data="menu:readings")],
        ]
    )
...
async def open_share_referral(
    ...
    logger.info(
        "share_referral_started share_format=%s renderer_version=%s",
        SHARE_FORMAT,
...
def render_share_preview(card: ShareCardPayload) -> str:
    return (
        ...
        + escape(_bounded_card_text(card))
...
def render_public_share(card: ShareCardPayload) -> str:
    ...
    raw = f"✨ {card.headline}\n\n{_bounded_card_text(card)}\n\n— Numa"
    if len(raw) <= MAX_PUBLIC_SHARE_LENGTH:
        return raw
    suffix = "…\n\n— Numa"
    return raw[: MAX_PUBLIC_SHARE_LENGTH - len(suffix)].rstrip() + suffix


def _bounded_card_text(card: ShareCardPayload) -> str:
    return card.short_text.strip()
