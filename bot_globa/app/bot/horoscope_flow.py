...
HOROSCOPE_TOPIC_LABELS = MappingProxyType(
    {
        "day_forecast": "☀️ Прогноз на сегодня",
...
HOROSCOPE_FLOW = ReadingFlow(
    ...
    texts=PersonaFlowTexts(
        welcome=(
            "🪐 Астрология\n\nВыберите, что именно посмотреть по карте: «Натальная карта», "
            "«Прогноз на сегодня», прогноз на неделю или месяц. Вопросы об отношениях и "
            "выборе можно просто рассказать Numa из главного меню."
        ),
        processing=(
