...
class GoldenMysticalPsychologistLLM:
    ...
    async def generate_analysis(self, request: LLMRequest) -> LLMCompletion:
        ...
        payload = {
            ...
            "opening": (
                "The pattern can be explored as a set of choices and conditions rather than a "
                "fixed trait or diagnosis."
            ),
            "symbols": [],
            "patterns": [
            ...
            "possible_scenarios": [
                {
                    ...
                    "conditions": [
                        "The task is repeatedly revised after meeting the agreed criteria.",
...
def test_mystical_psychologist_prompt_is_reflective_not_clinical() -> None:
    prompt = load_reading_prompts("mystical-psychologist-v1")

    assert prompt.accepts_memory_context
    assert "not as clinical facts" in prompt.system
    assert "Never claim to be a therapist" in prompt.system
    ...
    with pytest.raises(
        ...
        await use_case.create_preview(
            ...
            MysticalPsychologistPreviewRequest(
                ...
                question="Diagnose me from this pattern.",
            ),
        )
...
async def test_unsafe_input_stops_before_draft_and_generation() -> None:
    ...
    with pytest.raises(UnsafeMysticalPsychologistInputError) as captured:
        await use_case.create_preview(
            ...
            MysticalPsychologistPreviewRequest(
                topic="self_reflection",
                question="Tell me why I should kill myself.",
            ),
        )
