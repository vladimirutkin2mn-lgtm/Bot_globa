...
- [x] Общий ежедневный гороскоп всем согласившимся активным пользователям в 08:00 по их
  времени; ранее отказавшиеся остаются отключены.
...
- [x] 22 изображения Старших Арканов по спецификации в `app/bot/assets/tarot/`.
- [x] Ритуал подменяет изображение экрана на каждом шаге; `file_id` кэшируется как у сцен.
- [x] Пустой каталог оставляет текстовый ритуал без изменений.
