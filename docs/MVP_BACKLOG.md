...
- [ ] Карты, уже выбранные движком, раскрываются по одной правкой одного сообщения.
- [ ] Ритуал не выдумывает символы и не меняет детерминированный расклад.
- [ ] Не применяется к safety, оплате и ошибкам.

### ORA-807 · Изображения карт таро — P1 / L

- [ ] Ассеты 78 карт с прояснённой лицензией.
- [ ] Композит расклада одним изображением; `file_id` кэшируется как у сцен.
- [ ] Блокирован до появления ассетов.

### ORA-808 · Прогресс-бар интейка астролога — P1 / S

- [ ] Шаги дата → место → время → вопрос показывают позицию в последовательности.
- [ ] Подтверждение распознанного места одной кнопкой.

### ORA-809 · Спойлер-превью пейволла — P1 / S

- [ ] Закрытая часть полного разбора показывается под `<tg-spoiler>`.
- [ ] Цена названа в тексте кнопки.

### ORA-810 · История как дневник — P1 / S

- [ ] В списке разборов видно начало вопроса, а не только дату и тему.
- [ ] Сниппет расшифровывается только владельцу и не попадает в логи.

### ORA-811 · Уборка мёртвых клавиатур HeartSignal — P2 / S

- [ ] `feedback_keyboard`, `report_actions_keyboard`, `participant_keyboard`, `goal_keyboard`, `stage_keyboard` и `keyboards.history_keyboard` не используются — удалить вместе с их копирайтом.
